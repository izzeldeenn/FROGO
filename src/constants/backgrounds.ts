export const BACKGROUNDS = [
  { id: 'default', name: 'افتراضي', value: 'transparent' },
  { id: 'gradient1', name: 'تدرج أخضر', value: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  { id: 'gradient2', name: 'تدرج أزرق', value: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
  { id: 'gradient3', name: 'تدرج برتقالي', value: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
  { id: 'gradient4', name: 'تدرج بنفسجي', value: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
  { id: 'gradient5', name: 'تدرج رمادي', value: 'linear-gradient(135deg, #d299c2 0%, #fef9d7 100%)' },
  { id: 'pattern1', name: 'نقوش بسيط', value: 'repeating-linear-gradient(45deg, #606dbc 25%, transparent 25%), repeating-linear-gradient(-45deg, #606dbc 25%, transparent 25%)' },
  { id: 'pattern2', name: 'نقوش متقاطع', value: 'repeating-linear-gradient(90deg, #667eea 0%, #764ba2 50%, #667eea 100%)' },
  // New photo backgrounds from local folder
  { id: 'focus1', name: 'مشروم طبيعي', value: 'url("/backgrounds/photos/adege-mushroom-7570693.jpg")' },
  { id: 'focus2', name: 'جبال الدولوميت', value: 'url("/backgrounds/photos/cernetice-dolomites-5076487.jpg")' },
  { id: 'focus3', name: 'شجرة غابة', value: 'url("/backgrounds/photos/heckimg-tree-7619791.jpg")' },
  { id: 'focus4', name: 'شاطئ هادئ', value: 'url("/backgrounds/photos/idat-beach-7511632.jpg")' },
  { id: 'focus5', name: 'غروب الشمس', value: 'url("/backgrounds/photos/kanenori-sunset-7133867.jpg")' },
  { id: 'focus6', name: 'شاطئ صيفي', value: 'url("/backgrounds/photos/pexels-beach-1850059.jpg")' },
  { id: 'focus7', name: 'غروب ملون', value: 'url("/backgrounds/photos/pok_rie-sunset-1637376.jpg")' },
  // New Unsplash backgrounds
  { id: 'focus8', name: 'مركبات على قمة الجبل', value: 'url("/backgrounds/photos/hilltop-vehicles.jpg")' },
  { id: 'focus9', name: 'كثبان رملية صحراوية', value: 'url("/backgrounds/photos/desert-dunes.jpg")' },
  { id: 'focus10', name: 'غرفة مع شاشات', value: 'url("/backgrounds/photos/bedroom-screens.jpg")' },
  { id: 'focus11', name: 'مدينة حديثة', value: 'url("/backgrounds/photos/modern-city.jpg")' },
  { id: 'focus12', name: 'نخيل silhouetted', value: 'url("/backgrounds/photos/palm-trees.jpg")' }
];
